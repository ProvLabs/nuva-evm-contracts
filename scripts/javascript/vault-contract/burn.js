...
async function main() {
  ...
  try {
    ...
    console.log(
      `Attempting to burn ${ethers.formatUnits(AMOUNT_TO_BURN, 6)} tokens...`,
    );

    console.log("burner.address", burner.address);
    const hasBurnRole = await withdrawal.hasRole(
      withdrawal.BURN_ROLE(),
      burner.address,
    );
    console.log(
      `Does ${burner.address} have BURN_ROLE? ${hasBurnRole ? "✅ Yes" : "❌ No"}`,
    );

    const burnTx = await withdrawal
      .connect(burner)
