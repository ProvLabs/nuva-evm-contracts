...
async function main() {
    ...
    // Convert the hash to the Ethereum Signed Message format
    const messageHashBytes = ethers.getBytes(amlMessageHash);
    const message = ethers.hashMessage(messageHashBytes);
    
    // Sign the message with the AML signer's private key
    const amlSignature = await amlSigner.signMessage(ethers.getBytes(message));
