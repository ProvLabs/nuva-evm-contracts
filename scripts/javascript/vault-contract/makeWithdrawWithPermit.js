...
if (!WITHDRAWAL_FACTORY_ADDR) {
    throw new Error(
      "WITHDRAWAL_FACTORY_CONTRACT is not set."
    );
}
...
console.log("   ✅ AML Signature created.");
    ...
    try {
        ...
        const estimatedGas = await withdrawal.connect(user).withdrawWithPermit.estimateGas(
            amountToWithdraw,
            amlSignature,
            amlDeadline,
            permitDeadline,
        ...
        if (error.data) {
            ...
            if (error.data.data) {
                ...
                if (revertData.length > 10) { // 0x + 4 bytes (8 chars) + at least 2 chars of data
                    ...
                    if (errorSignature === '0xd1cc7385') {
                        ...
                        console.error("This usually means the permit signature verification failed or the allowance wasn't set correctly.");
