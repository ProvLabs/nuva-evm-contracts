...
async function main() {
  ...
  if (onChainSigner.toLowerCase() !== scriptSigner.toLowerCase()) {
    ...
    console.error(
      "   The AML_PRIVATE_KEY in your .env file does not match the",
    );
  ...
  try {
    ...
    console.log(
      "❌ Token may not fully support EIP-2612 (DOMAIN_SEPARATOR not found)",
    );
  }
      ...
      if (error.data.data) {
        ...
        if (revertData.length > 10) {
          ...
          if (errorSignature === "0xd1cc7385") {
            ...
            console.error(
              "1. The token contract's permit function implementation",
            );
