...
async function main() {
  ...
  const DEPOSITOR_FACTORY_CONTRACT_ADDR =
    process.env.DEPOSITOR_FACTORY_CONTRACT;
  ...
  const depositorFactory = await ethers.getContractAt(
    "DepositorFactory",
    DEPOSITOR_FACTORY_CONTRACT_ADDR,
  );
  const depositor = await ethers.getContractAt(
    "Depositor",
    DEPOSITOR_CONTRACT_ADDR,
  );
  ...
  console.log(
    "Depositor address:",
    await depositorFactory.depositors(shareTokenAddress, depositTokenAddress),
  );

  console.log(
    "Factory implementation address:",
    await depositorFactory.implementation(),
  );
  ...
  if (cloneAddress === "") {
    console.error(
      "Could not find 'DepositorCreated' event in transaction logs.",
    );
  }
