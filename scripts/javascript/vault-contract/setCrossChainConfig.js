...
async function main() {
    ...
    const setCrossChainConfigTx = await depositor.connect(user).setCrossChainConfig(CROSS_CHAIN_VAULT_ADDRESS);
    const receipt = await setCrossChainConfigTx.wait();

    console.log("✅ setCrossChainConfig successful! Transaction hash:", receipt.hash);
