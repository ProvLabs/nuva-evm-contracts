...
async function main() {
  ...
  console.log(
    `   Status Before: ${hasRoleBefore ? "✅ Has Role" : "❌ No Role"}`,
  );

  if (!hasRoleBefore) {
    console.log("⚠️ Address does not have the role. Skipping transaction.");
    return;
  }
  ...
  const tx = await withdrawal.connect(burner).revokeRole(BURN_ROLE_HASH, addr);
  ...
  console.log(
    `   Status After:  ${hasRoleAfter ? "❌ FAILED TO UPDATE" : "✅ No Role"}`,
  );
}
