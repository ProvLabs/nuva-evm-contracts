...
async function main() {
  ...
  const oldCloneAddress = await factory.withdrawals(
  ...
  if (oldCloneAddress === ethers.ZeroAddress) {
    throw new Error(
      "No existing withdrawal found for this pair. Nothing to migrate.",
    );
  }
  ...
  for (const log of receipt.logs) {
    ...
    if (parsedLog && parsedLog.name === "WithdrawalMigrated") {
      ...
      if (parsedLog.args.oldWithdrawal !== oldCloneAddress) {
        console.warn(
          "Warning: Event 'oldWithdrawalAddress' does not match the one we had mapped.",
        );
      }
      break;
    }
  ...
  if (newCloneAddress === "") {
    throw new Error(
      "Migration failed: Could not find 'WithdrawalMigrated' event in transaction logs.",
    );
  }

  // Final check: Query the map again to ensure it was updated
  const mappedAddress = await factory.withdrawals(
    PAYMENT_TOKEN_ADDRESS,
    SHARE_TOKEN_ADDRESS,
  );
  ...
  if (mappedAddress !== newCloneAddress) {
    throw new Error(
      "CRITICAL ERROR: The 'withdrawals' map was not updated correctly!",
    );
  }
