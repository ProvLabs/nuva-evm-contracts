...
if (!TARGET_KEY || !AMOUNT_TO_WITHDRAW_STRING) {
  ...
  console.error(
    "Usage via Flags:   npx hardhat run script.js -- --target <NAME> --amount <AMOUNT>",
  );
  console.error(
    "Usage via Env Var: TARGET=<NAME> AMOUNT=<AMOUNT> npx hardhat run script.js",
  );
...
if (!config) {
  ...
  console.error(
    `Please use one of: ${Object.keys(TARGET_CONFIG).join(", ")}\n`,
  );
  process.exit(1);
}
...
async function main() {
  ...
  if (balance < amountToWithdraw) {
    console.error(
      `❌ Error: User does not have enough tokens. Needs ${AMOUNT_TO_WITHDRAW_STRING}.`,
    );
    console.error(
  ...
  console.log(
    `Current allowance for withdrawer: ${ethers.formatUnits(currentAllowance, TOKEN_DECIMALS)}`,
  );
  ...
  console.log(
    `Clone contract balance (shares): ${ethers.formatUnits(finalBalance, TOKEN_DECIMALS)}`,
  );
}
