...
if (!TARGET_KEY || !AMOUNT_TO_DEPOSIT_STRING) {
  ...
  console.error(
    "Usage: npx hardhat run script.js -- --target <NAME> --amount <AMOUNT>",
  );
  console.error(`Valid targets: ${Object.keys(CLONE_MAPPING).join(", ")}`);
  console.error(
    "Example: npx hardhat run script.js -- --target nvylds --amount 0.222\n",
  );
  process.exit(1);
}
...
if (!CLONE_ADDRESS) {
  ...
  console.error(
    `Please use one of the following: ${Object.keys(CLONE_MAPPING).join(", ")}\n`,
  );
  process.exit(1);
}
...
async function main() {
  ...
  if (balance < amountToDeposit) {
    console.error(
      `❌ Error: User does not have enough tokens. Needs ${AMOUNT_TO_DEPOSIT_STRING}.`,
    );
    console.error(
      `  User Balance: ${ethers.formatUnits(balance, TOKEN_DECIMALS)}`,
    );
    process.exit(1);
  }
  console.log(
    `User balance is: ${ethers.formatUnits(balance, TOKEN_DECIMALS)} tokens.`,
  );
  ...
  console.log(
    `User token balance: ${ethers.formatUnits(userBalance, TOKEN_DECIMALS)}`,
  );
  ...
  console.log(
    `Current allowance for depositor: ${ethers.formatUnits(currentAllowance, TOKEN_DECIMALS)}`,
  );
  ...
  console.log(
    `Destination wallet balance: ${ethers.formatUnits(finalBalance, TOKEN_DECIMALS)} tokens.`,
  );
}
