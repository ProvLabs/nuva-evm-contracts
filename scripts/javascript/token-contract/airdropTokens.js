...
async function main() {
  ...
  console.log(
    `Airdropping ${AMOUNT_PER_RECIPIENT_ETH} ETH (${amountInWei.toString()} Wei) to each address.`,
  );
  ...
  if (funderBalance < totalCost) {
    console.error(
      "❌ Error: Funder account does not have enough ETH to complete the airdrop.",
    );
  ...
  const finalFunderBalance = await hre.ethers.provider.getBalance(
    funder.address,
  );
  console.log(
    `Funder final balance: ${hre.ethers.formatEther(finalFunderBalance)} ETH`,
  );
}
