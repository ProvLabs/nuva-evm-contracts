...
async function main() {
  ...
  if (
    ...
    !ethers.isAddress(OLD_ADDRESS) ||
    !ethers.isAddress(NEW_ADDRESS)
  ) {
    console.error(
      "Invalid address found in configuration. Please check your addresses.",
    );
    process.exit(1);
  }
  ...
  try {
    ...
    console.log(
      "Warning: Role was granted to new address but could not be revoked from old address.",
    );
    process.exit(1);
  }
