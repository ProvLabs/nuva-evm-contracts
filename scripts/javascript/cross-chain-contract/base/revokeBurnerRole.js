...
async function main() {
    ...
    try {
        const hasBurnRole = await crossChainManager.hasRole(burnRole, burnAdmin.address);
        if (hasBurnRole) {
            console.log(`Revoking burner role to address: ${address}...`);
            const revokeTx = await crossChainManager.connect(burnAdmin).revokeRole(burnRole, address);
            const receipt = await revokeTx.wait();
            console.log("✅ Burn role revoked successfully! Transaction hash:", receipt.hash);
        } else {
