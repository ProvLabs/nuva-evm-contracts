...
async function main() {
    ...
    try {
        const hasBurnRole = await crossChainManager.hasRole(burnRole, owner.address);
        if (hasBurnRole) {
            console.log(`Revoking burner role to address: ${address}...`);
            const revokeTx = await crossChainManager.connect(owner).revokeRole(burnRole, address);
