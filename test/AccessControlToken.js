...
describe("CustomToken AccessControl", function () {
  async function deploy() {
    ...
    await expect(
      token.connect(admin).grantRole(MINTER_ROLE, await admin.getAddress()),
    ).to.emit(token, "RoleGranted");
    const scale = BigInt(10 ** decimals);
    await token
      .connect(admin)
      .mint(await admin.getAddress(), initialSupply * scale);
    await expect(
      token.connect(admin).revokeRole(MINTER_ROLE, await admin.getAddress()),
  ...
  it("assigns DEFAULT_ADMIN, MINTER to creator", async function () {
    ...
    expect(
      await token.hasRole(MINTER_ADMIN_ROLE, await admin.getAddress()),
    ).to.equal(true);
    expect(await token.hasRole(MINTER_ROLE, await admin.getAddress())).to.equal(
      false,
    );
  });
  ...
  it("mint requires MINTER_ROLE and is transferrable via grant/revoke", async function () {
    ...
    await expect(
      token.connect(alice).mint(await alice.getAddress(), amount),
    ).to.be.revertedWithCustomError(token, "AccessControlUnauthorizedAccount");

    await expect(
      token.connect(admin).grantRole(MINTER_ROLE, await alice.getAddress()),
    ).to.emit(token, "RoleGranted");
    ...
    await expect(
      token.connect(admin).revokeRole(MINTER_ROLE, await alice.getAddress()),
    ).to.emit(token, "RoleRevoked");
