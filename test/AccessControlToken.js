...
describe("CustomToken AccessControl", function () {
    ...
    it("mint requires MINTER_ROLE and is transferrable via grant/revoke", async function () {
        ...
        await expect(token.connect(admin).grantRole(MINTER_ROLE, await alice.getAddress())).to.emit(
            token,
            "RoleGranted",
        );

        await expect(token.connect(alice).mint(await alice.getAddress(), amount)).to.emit(token, "Transfer");

        await expect(token.connect(admin).revokeRole(MINTER_ROLE, await alice.getAddress())).to.emit(
            token,
            "RoleRevoked",
        );
