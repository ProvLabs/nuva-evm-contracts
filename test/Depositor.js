...
describe("Depositor", function () {
  async function deployFixture() {
    ...
    await (await depositorFactory.createDepositor(
      shareToken,
  ...
  it("deposit with prior approve transfers tokens and emits event", async function () {
    ...
    await expect(depositor.connect(user).deposit(
      amt,
      await destination.getAddress(),
      signature,
      deadline
    )).to.emit(depositor, "Deposit").withArgs(
  ...
  it("validates amount and destination", async function () {
    ...
    const build = async (amount, dest) => buildAmlSignature({
      amlSigner,
      user,
      depositToken: token,
      shareToken,
  ...
  it("depositWithPermit performs permit then transfers in one tx", async function () {
    ...
    await expect(depositor.connect(user).depositWithPermit(
      amt,
      await destination.getAddress(),
      signature,
      nowTs + 3600n,
