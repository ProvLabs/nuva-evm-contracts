...
describe("CrossChainManager", function () {
  ...
  beforeEach(async function () {
    ...
    await crossChainManager
      .connect(owner)
      .addDestinationAddress(destinationAddress.address);
  ...
  describe("Deposit", function () {
    ...
    it("Should reject deposit with invalid destination", async function () {
      ...
      await crossChainManager
        .connect(owner)
        .addDestinationAddress(user2.address);
      ...
      await crossChainManager
        .connect(owner)
        .removeDestinationAddress(user2.address);
  ...
  describe("Withdraw", function () {
    ...
    it("Should reject withdraw with zero amount", async function () {
      ...
      await expect(
        crossChainManager
          .connect(user1)
          .withdraw(amount, destinationAddress.address, amlSignature, deadline),
      ).to.be.revertedWithCustomError(
        crossChainManager,
    ...
    it("Should reject withdraw with insufficient shareToken balance", async function () {
      ...
      await expect(
        crossChainManager
          .connect(user1)
          .withdraw(WITHDRAW_AMOUNT, user2.address, amlSignature, deadline),
      )
  ...
  describe("Admin Functions", function () {
    ...
    it("Should allow owner to add destination address", async function () {
      ...
      await expect(
        crossChainManager.connect(owner).addDestinationAddress(newDestination),
      )
    ...
    it("Should allow owner to remove destination address", async function () {
      await expect(
        crossChainManager
          .connect(owner)
          .removeDestinationAddress(destinationAddress.address),
      )
        .to.emit(crossChainManager, "DestinationAddressRemoved")
        .withArgs(destinationAddress.address);

      expect(await crossChainManager.isDestination(destinationAddress.address))
        .to.be.false;
    });
  ...
  describe("Edge Cases", function () {
    it("Should handle multiple destination addresses", async function () {
      ...
      await crossChainManager
        .connect(owner)
        .addDestinationAddress(destination2);
