...
describe("CustomToken ERC20Permit", function () {
  ...
  it("nonces start at 0 and increment after permit", async function () {
    ...
    await expect(token.permit(ownerAddr, spenderAddr, value, deadline, v, r, s))
  ...
  it("reverts when deadline has passed", async function () {
    ...
    await expect(token.permit(ownerAddr, spenderAddr, value, pastDeadline, v, r, s))
      .to.be.reverted;
  });
  ...
  it("reverts with invalid signer (signature not from owner)", async function () {
    ...
    await expect(token.permit(ownerAddr, spenderAddr, value, deadline, sig.v, sig.r, sig.s))
      .to.be.reverted;
  });
});
