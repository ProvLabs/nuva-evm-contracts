...
describe("DepositorFactory", function () {
  ...
  before(async function () {
    ...
    await expect(
      shareToken.connect(owner).grantRole(MINTER_ROLE, ownerAddress),
    ).to.emit(shareToken, "RoleGranted");
    ...
    await expect(
      depositToken.connect(owner).grantRole(MINTER_ROLE, ownerAddress),
    ).to.emit(depositToken, "RoleGranted");
  ...
  it("should not allow migrating non-existent depositor", async function () {
    const ownerAddress = await owner.getAddress();
    const newShareToken = await Token.deploy(
      "New Share",
      "NSHR",
      ownerAddress,
      18,
